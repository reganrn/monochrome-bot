 ...
 */
async function resolve(query, requestedBy = null) {
  const q = query.trim();

  // ── TIDAL track URL ─────────────────────────────────────────────────────────
  const tidalTrack = q.match(/tidal\.com\/(?:browse\/)?track\/(\d+)/i);
  if (tidalTrack) {
    const t = await hifi.track(tidalTrack[1]);
  ...
  if (tidalAlbum) {
    const { tracks } = await hifi.album(tidalAlbum[1]);
  ...
  if (tidalArtist) {
    const { topTracks } = await hifi.artist(tidalArtist[1]);
  ...
  if (tidalPlaylist) {
    const tracks = await hifi.playlist(tidalPlaylist[1]).catch(() => []);
    if (tracks.length) {
      tracks.forEach(t => { if (t) t.requestedBy = requestedBy; });
      return tracks.filter(Boolean);
    }
  ...
  if (monoTrack) {
    const t = await hifi.track(monoTrack[1]);
    if (!t) throw new Error('Track not found.');
    t.requestedBy = requestedBy;
    return [t];
  }
  ...
  if (monoAlbum) {
    const { tracks } = await hifi.album(monoAlbum[1]);
    tracks.forEach(t => { if (t) t.requestedBy = requestedBy; });
    return tracks.filter(Boolean);
  }

  // ── Plain text → Hi-Fi search ───────────────────────────────────────────────
  const results = await hifi.search(q, 1);
 ...
 * @returns {Promise<TrackInfo[]>}
 */
async function searchResults(query, limit = 10) {
  const results = await hifi.search(query, limit);
